...
class CubatureKalmanFilter(object):
    ...
    def predict_propogate(self, x,P, horizon,dt=None, fx_args=()):
        r""" Performs the predict step of the CKF. On return, self.x and
        self.P contain the predicted state (x) and covariance (P).
        ...
        sigmas_f = deepcopy(self.sigmas_f)

        sigmas_x = np.zeros((horizon,self._num_sigmas,self.dim_x))

        for step in range(horizon):
            sigmas = spherical_radial_sigmas(x, P)

            sigmas_x[step] = sigmas

            # evaluate cubature points
            for k in range(self._num_sigmas):
                sigmas_f[k] = self.fx(sigmas[k], dt, *fx_args)

            x, P = ckf_transform(sigmas_f, self.Q)
