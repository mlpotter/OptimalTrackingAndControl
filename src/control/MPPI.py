...
def MPPI(U_nominal,chis_nominal,U_ptb,ps,time_step_sizes,limits):
    ...
    N,N_C,T = U_nominal.shape

    # U is Number of sensors x Control Inputs x T
    U_upper = jnp.ones((1,1,N_C, T)) * jnp.reshape(limits[0],(1,1,N_C,1))
    U_lower = jnp.ones((1,1,N_C, T)) * jnp.reshape(limits[1],(1,1,N_C,1))
    ...
    U_MPPI = jnp.clip(U_nominal + U_ptb,U_lower,U_upper)
...
def MPPI_ptb(stds,N, time_steps, num_traj, key):
    ...
    U_velocity = jax.random.uniform(key, shape=(num_traj, N, 1, time_steps), minval=v_min, maxval=v_max)
    U_angular_velocity = jax.random.uniform(key, shape=(num_traj, N, 1, time_steps), minval=av_min,
                                            maxval=av_max)
...
def MPPI_scores(score_fn,ps,qs,U_MPPI,chis,time_step_sizes,A, Q, Js,paretos,Pt, Gt, Gr, L, lam, rcs,s,gamma):
    ...
    MPPI_score_fn = vmap(lambda u: score_fn(u, chis, ps, qs, time_step_sizes=time_step_sizes,
